...
\begin{definition}
Una \(CMTD\) es \textbf{irreducible} cuando todos sus estados están comunicados entre sí. Un conjunto de estados en S se dice irreducible cuando no contiene ningún subconjunto cerrado. Si la \(CMTD\) no es irreducible, se llama \textbf{reducible}.
...
\begin{definition}
Sea \(\{X_n, n \in \mathbb{N}\}\) una \(CMTD\) homogénea con espacio de estados \(S = \{1, 2,...,N\}\). Se definen las \textbf{probabilidades de primera visita} o primer paso del estado \(i\) al \(j\) en \(n\) transiciones, con \(f_{ij}(n)\),
...
\begin{definition}
\protect\hypertarget{def:irreducible}{}\label{def:irreducible}Una \(CMTD\) \(\{X_n, n \in \mathbb{N}\}\) con espacio de estados \(S\) se dice que es \textbf{irreducible} si para cada pareja de estados \(i\) y \(j\) en \(S\), existe algún instante de tiempo \(k>0\) en el que es posible llegar a \(j\) desde \(i\), esto es,
...
Una \(CMTD\) que no es irreducible se denomina \textbf{reducible}.

\begin{whitebox}
Podemos estudiar esta característica del proceso mediante un análisis descriptivo del mismo, con la función \texttt{summary()} de la librería \texttt{markovchain}, como ya vimos en el Ejemplo \ref{exm:summary-markovchain}.
...
\begin{definition}
\protect\hypertarget{def:periodo}{}\label{def:periodo}Sea la \(CMTD\) \(\{X_n, n \in \mathbb{N}\}\) con espacio de estados \(S =\{1, 2,..., N\}\) y \(d\) el entero más grande tal que para cualquier estado \(i \in S\)
...
\NormalTok{s}\OtherTok{=}\FunctionTok{sd}\NormalTok{(noperaciones)}
\CommentTok{\# calculamos la media y el error}
\FunctionTok{cat}\NormalTok{(}\StringTok{"E(operaciones en 8 horas)="}\NormalTok{,}\FunctionTok{round}\NormalTok{(m,}\DecValTok{2}\NormalTok{), }\StringTok{", Error="}\NormalTok{,}\FunctionTok{round}\NormalTok{(s,}\DecValTok{2}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{s}\OtherTok{=}\FunctionTok{sqrt}\NormalTok{(}\FunctionTok{sum}\NormalTok{((favorables}\SpecialCharTok{{-}}\NormalTok{p)}\SpecialCharTok{\^{}}\DecValTok{2}\NormalTok{)}\SpecialCharTok{/}\NormalTok{(nsim}\SpecialCharTok{\^{}}\DecValTok{2}\NormalTok{))}
\CommentTok{\# calculamos la media y el error}
\FunctionTok{cat}\NormalTok{(}\StringTok{"Pr(sin operaciones en 1 hora)="}\NormalTok{,p, }\StringTok{", Error="}\NormalTok{,s)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{cat}\NormalTok{(}\StringTok{"}\SpecialCharTok{\textbackslash{}n}\StringTok{ IC(Pr(sin operaciones en 1 hora))=["}\NormalTok{,}\FunctionTok{round}\NormalTok{(p}\SpecialCharTok{{-}}\FunctionTok{qnorm}\NormalTok{(}\FloatTok{0.975}\NormalTok{)}\SpecialCharTok{*}\NormalTok{s,}\DecValTok{3}\NormalTok{), }\StringTok{","}\NormalTok{,}\FunctionTok{round}\NormalTok{(p}\SpecialCharTok{+}\FunctionTok{qnorm}\NormalTok{(}\FloatTok{0.975}\NormalTok{)}\SpecialCharTok{*}\NormalTok{s,}\DecValTok{3}\NormalTok{),}\StringTok{"]"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{s}\OtherTok{=}\FunctionTok{sqrt}\NormalTok{(}\FunctionTok{sum}\NormalTok{((favorables}\SpecialCharTok{{-}}\NormalTok{p)}\SpecialCharTok{\^{}}\DecValTok{2}\NormalTok{)}\SpecialCharTok{/}\NormalTok{(nsim}\SpecialCharTok{\^{}}\DecValTok{2}\NormalTok{))}
\CommentTok{\# calculamos la media y el error}
\FunctionTok{cat}\NormalTok{(}\StringTok{"Pr(suceso)="}\NormalTok{,p, }\StringTok{", Error="}\NormalTok{,s)}
\end{Highlighting}
\end{Shaded}
